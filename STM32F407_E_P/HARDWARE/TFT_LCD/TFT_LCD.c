...
{ 	
  ...
  readWriteTiming.FSMC_AccessMode = FSMC_AccessMode_A;	 //模式A 
	...
	writeTiming.FSMC_AddressSetupTime =12;	      //地址建立时间（ADDSET）为12个HCLK =54ns 
  writeTiming.FSMC_AddressHoldTime = 0x00;	 //地址保持时间（A		
  writeTiming.FSMC_DataSetupTime =14;		 //数据保存时间为6ns*9个HCLK=54ns
...
void TFT_LCD_ShowLogo(void)
{
  ...
  volatile u16 Color[554];
	...
	for(i=0;i<82;i++)
	{
		...
		{
			index=1108*i+2*j;
			Color[j]=(gImage_LOGO[index+1]<<8)+gImage_LOGO[index];
//			Color[j]=RED;
		}
		TFT_LCD_SetCursor(x,y+i);
		TFT_LCD_SetCursor(x,y+i);
...
void TFT_LCD_ShowLASER(u16 x,u16 y)
{
	u16 j;
  volatile u16 Color[90];
	u8 i;
	u32 index=0;
	for(i=0;i<30;i++)
	{
		...
		{
			index=180*i+2*j;
			Color[j]=(gImage_LASER[index+1]<<8)+gImage_LASER[index];
		}
		TFT_LCD_SetCursor(x,y+i);
		TFT_LCD_SetCursor(x,y+i);
		TFT_LCD_WriteRAM_Prepare(); 
...
void TFT_LCD_ShowPOWER(u16 x,u16 y)
{
	u16 j;
  volatile u16 Color[95];
	u8 i;
	u32 index=0;
	for(i=0;i<30;i++)
	{
		...
		{
			index=190*i+2*j;
			Color[j]=(gImage_POWER[index+1]<<8)+gImage_POWER[index];
		}
		TFT_LCD_SetCursor(x,y+i);
		TFT_LCD_SetCursor(x,y+i);
		TFT_LCD_WriteRAM_Prepare(); 
...
void TFT_LCD_ShowPULSE(u16 x,u16 y)
{
	u16 j;
  volatile u16 Color[90];
	u8 i;
	...
	for(i=0;i<30;i++)
	{
		...
		{
			index=180*i+2*j;
			Color[j]=(gImage_PULSE[index+1]<<8)+gImage_PULSE[index];
		}
		TFT_LCD_SetCursor(x,y+i);
		TFT_LCD_SetCursor(x,y+i);
		TFT_LCD_WriteRAM_Prepare(); 
...
void TFT_LCD_ShowRED(u16 x,u16 y)
{
	u16 j;
  volatile u16 Color[60];
	u8 i;
	u32 index=0;
	for(i=0;i<30;i++)
	{
		...
		{
			index=120*i+2*j;
			Color[j]=(gImage_RED[index+1]<<8)+gImage_RED[index];
		}
		TFT_LCD_SetCursor(x,y+i);
		TFT_LCD_SetCursor(x,y+i);
		TFT_LCD_WriteRAM_Prepare(); 
...
void TFT_LCD_ShowMODE(u16 x,u16 y)
{
	u16 j;
  volatile u16 Color[90];
	u8 i;
	...
	for(i=0;i<30;i++)
	{
		...
		{
			index=180*i+2*j;
			Color[j]=(gImage_MODE[index+1]<<8)+gImage_MODE[index];
		}
		TFT_LCD_SetCursor(x,y+i);
		TFT_LCD_SetCursor(x,y+i);
		TFT_LCD_WriteRAM_Prepare(); 
...
void TFT_LCD_ShowWIDTH(u16 x,u16 y)
{
	u16 j;
  volatile u16 Color[95];
	u8 i;
	...
	for(i=0;i<30;i++)
	{
		...
		{
			index=190*i+2*j;
			Color[j]=(gImage_WIDTH[index+1]<<8)+gImage_WIDTH[index];
		}
		TFT_LCD_SetCursor(x,y+i);
		TFT_LCD_SetCursor(x,y+i);
		TFT_LCD_WriteRAM_Prepare(); 
...
void TFT_LCD_ShowON(u16 x,u16 y)
{
	u16 j;
  volatile u16 Color[90];
	u8 i;
	...
	for(i=0;i<30;i++)
	{
		...
		{
			index=90*i+2*j;
			Color[j]=(gImage_ON[index+1]<<8)+gImage_ON[index];
		}
		for(j=45;j<90;j++)
		{
			Color[j]=BACK_COLOR;
...
void TFT_LCD_ShowOFF(u16 x,u16 y)
{
	u16 j;
  volatile u16 Color[90];
	u8 i;
	...
	for(i=0;i<30;i++)
	{
		...
		{
			index=108*i+2*j;
			Color[j]=(gImage_OFF[index+1]<<8)+gImage_OFF[index];
		}
		for(j=54;j<90;j++)
		{
			Color[j]=BACK_COLOR;
...
void TFT_LCD_ShowChar(u16 x,u16 y,u8 *InfNum,u8 Num)
{
	u16 j,s=0;
  volatile u16 Color[140];
	u8 i,k;
	volatile u32 index=0;
	for(i=0;i<30;i++)
	{
		for(k=0;k<Num;k++)
		{
			if(InfNum[k]<=9)
			{
				for(j=0;j<14;j++)
				{
					index=280*i+InfNum[k]*28+2*j;
					Color[s]=(gImage_Char[index+1]<<8)+gImage_Char[index];
					s++;
				}
			}
			if(InfNum[k]==10)
...
void ShowLine(void)
{
	...
	for(i=0;i<8;i++)
	{
		TFT_LCD_SetCursor(40,127+i);	//设置光标位置 
		TFT_LCD_WriteRAM_Prepare();
...
void TFT_LCD_ShowADNum(u8 Value)
{
  u8 i;
	...
	if(Num==1000)
	{
	 ...
	 TFT_LCD_ShowChar(AddressX,AddressY,InfNum,7);
...
void TFT_LCD_ShowPulseValue(u16 Value)
{
	...
	TFT_LCD_ShowChar(AddressX,AddressY,InfNum,4);
}
void TFT_LCD_ShowRedValue(u8 Value)
{
	if(Value==1)
	{
		TFT_LCD_ShowON(500,267);
	}
	else
	{
		TFT_LCD_ShowOFF(500,267);
	}
}
void TFT_LCD_ShowLaserValue(u8 Value)
{
	...
	if(Value==1)
	{
		TFT_LCD_ShowON(500,322);
	}
	else
	{
		TFT_LCD_ShowOFF(500,322);
	}
}
void TFT_LCD_ShowModeValue(u8 Value)
{
	u8 InfNum[8];
	u16 AddressX=500,AddressY=377;
	if(Value==1)
	{
		InfNum[0]=2;
	}
	...
	TFT_LCD_ShowChar(AddressX,AddressY,InfNum,1);
}
void TFT_LCD_ShowWidthValue(u16 Value)
{
	...
	TFT_LCD_ShowChar(AddressX,AddressY,InfNum,3);
}
